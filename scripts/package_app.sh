...
if [[ -f "$ICON_SRC" ]]; then
  ...
  if command -v magick >/dev/null 2>&1 || command -v convert >/dev/null 2>&1; then
    ...
    # Convert to grayscale, make near-white transparent, thicken at full res, then downscale crisply and sharpen
    $IM "$ICON_PROCESSED" -colorspace Gray -alpha on -fuzz 5% -transparent white -channel A -morphology Dilate Disk:1 +channel -filter point -resize 18x18 -unsharp 0x1 "$STATUS_OUT1" >/dev/null 2>&1 || true
    $IM "$ICON_PROCESSED" -colorspace Gray -alpha on -fuzz 5% -transparent white -channel A -morphology Dilate Disk:1 +channel -filter point -resize 36x36 -unsharp 0x1 "$STATUS_OUT2" >/dev/null 2>&1 || true
  else
